...
int main()
{
	int funkcija, br_iter = 10000, br_pon = 30, n, e;
	double omega, ro_p, ro_g;
	int dim, i;
	...
	for( funkcija = 1 ; funkcija < 4 ; funkcija++ )
	{
		...
		for( n = 50 ; n <= 200 ; n = n + 50 )
		{
			...
			for( i = 0 ; i < br_pon ; i++ )
			{
				...
				fprintf(opso, "\t\t%12.10f\t%lf\t%d\t%lf\n", g_best_opso, f_best, tren_iter_opso, cpu_time_used);
